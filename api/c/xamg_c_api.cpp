...
int XAMG_param_add_value_i(char *solver_type, char *key, uint32_t value, XAMG_param_id pid) {
    helper::xamg_param_add_value<uint32_t>(solver_type, key, value, pid);
    return 0;
}

int XAMG_param_add_value_d(char *solver_type, char *key, double value, XAMG_param_id pid) {
    helper::xamg_param_add_value<double>(solver_type, key, value, pid);
    return 0;
}

int XAMG_param_add_value_b(char *solver_type, char *key, uint8_t value, XAMG_param_id pid) {
    helper::xamg_param_add_value<bool>(solver_type, key, (bool)value, pid);
    return 0;
}
...
int XAMG_param_change_value_i(char *solver_type, char *key, uint32_t value, XAMG_param_id pid) {
    helper::xamg_param_change_value<uint32_t>(solver_type, key, value, pid);
    return 0;
}

int XAMG_param_change_value_d(char *solver_type, char *key, double value, XAMG_param_id pid) {
    helper::xamg_param_change_value<float64_t>(solver_type, key, value, pid);
    return 0;
}

int XAMG_param_change_value_b(char *solver_type, char *key, uint8_t value, XAMG_param_id pid) {
    helper::xamg_param_change_value<bool>(solver_type, key, (bool)value, pid);
    return 0;
}
...
int XAMG_param_forced_change_value_i(char *solver_type, char *key, uint32_t value,
                                     XAMG_param_id pid) {
    helper::xamg_param_forced_change_value<uint32_t>(solver_type, key, value, pid);
    return 0;
}

int XAMG_param_forced_change_value_d(char *solver_type, char *key, double value,
                                     XAMG_param_id pid) {
    helper::xamg_param_forced_change_value<float64_t>(solver_type, key, value, pid);
    return 0;
}

int XAMG_param_forced_change_value_b(char *solver_type, char *key, uint8_t value,
                                     XAMG_param_id pid) {
    helper::xamg_param_forced_change_value<bool>(solver_type, key, (bool)value, pid);
    return 0;
}
...
int XAMG_csr_matrix_alloc(uint64_t nrows, uint64_t ncols, uint64_t nnz, XAMG_matrix_id mcsrid) {
    auto &mcsr = mcsrlist[mcsrid];

    mcsr->nrows = nrows;
    mcsr->ncols = ncols;
    mcsr->block_nrows = mcsr->nrows;
    mcsr->block_ncols = mcsr->ncols;
    mcsr->nonzeros = nnz;
    mcsr->alloc();
    return 0;
}
...
int XAMG_solver_print_info(XAMG_solver_id sid) {
    auto &sol = slist[sid];
...
int XAMG_solver_get_iter_info(int *iter_info, XAMG_solver_id sid) {
    auto &sol = slist[sid];
    sol->get_stats();

    iter_info[0] = sol->stats.iters;
    iter_info[1] = sol->stats.sub_iters;
    return 0;
}
