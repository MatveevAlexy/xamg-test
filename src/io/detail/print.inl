...
void io::print_hline(const uint16_t nv) {
    uint16_t delimiter_len = 17 + nv * (2 * 15);
...
void io::print_residuals_header(uint16_t res_type, const uint16_t nv) {
    ...
    XAMG::out << XAMG::CONVERGENCE;
    XAMG::out << std::setfill(' ') << std::setw(5) << "Type"
              << " | ";
    XAMG::out << std::setfill(' ') << std::setw(6) << "Iter"
              << " | ";
    ...
    XAMG::out << XAMG::CONVERGENCE;
    XAMG::out << std::setfill(' ') << std::setw(14) << "NV"
              << " | ";
    for (uint16_t i = 0; i < nv; ++i)
...
void io::print_residuals_delimiter(const uint16_t nv) {
    print_hline(nv);
}
...
template <typename T>
void io::print_residuals(const std::string &prefix, const uint32_t iter,
                         const std::vector<T> &res_elem, const std::vector<T> &res0_elem,
                         const std::vector<bool> &conv_status) {
    XAMG::out << std::scientific;

    XAMG::out << XAMG::CONVERGENCE;
    XAMG::out << std::setfill(' ') << std::setw(5) << prefix << " | " << std::setw(6) << iter
              << " | ";

    for (uint16_t nv = 0; nv < res_elem.size(); nv++) {
        if (!conv_status[nv]) {
            XAMG::out << std::setw(12) << sqrt(res_elem[nv]) << " | " << std::setw(12)
                      << sqrt(res_elem[nv] / res0_elem[nv]) << " | ";
        } else {
