...
struct logout : std::ostream, std::streambuf {
    ...
    bool env_settings_applied = false;
    ...
    void get_types_mask(const std::string &logopts = "") {
        uint32_t add_mask = 0, remove_mask = 0;
        std::string env;
        if (logopts == "") {
            char *env_ = getenv("XAMG_LOG");
            if (!env_)
                return;
            env = env_;
            env_settings_applied = true;
        } else {
            env = logopts;
        }
    ...
    void set_output(const std::string &name, const std::string &logopts = "") {
        assert(!out_created);
        ...
        if (!env_settings_applied && logopts != "") {
            get_types_mask(logopts);
        }
    }
    ~logout() {
        if (out) {
