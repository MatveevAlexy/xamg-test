...
struct hypre_base {
    ...
    void create_bicgstab_solver(HYPRE_Solver &sol, const params::global_param_list &global_list) {
        ...
        XAMG::misc::str_split(list.get_string("convergence_details"), ',', conv);

        HYPRE_ParCSRBiCGSTABCreate(*((MPI_Comm *)id.get_comm()), &sol);

        //  Set some parameters (See Reference Manual for more parameters)
        HYPRE_BiCGSTABSetMaxIter(sol, list.get_int("max_iters"));
        HYPRE_BiCGSTABSetTol(sol, list.get_float("rel_tolerance"));
        HYPRE_BiCGSTABSetAbsoluteTol(sol, list.get_float("abs_tolerance"));
    ...
    void create_multigrid_solver(HYPRE_Solver &sol, const params::global_param_list &global_list) {
        ...
        HYPRE_BoomerAMGSetMaxIter(sol, list.get_int("max_iters"));
        HYPRE_BoomerAMGSetTol(sol, list.get_float("rel_tolerance"));

        bool hypre_log = list.get_bool("hypre_log");
        if (hypre_log) {
            HYPRE_BoomerAMGSetPrintLevel(sol, 3);
            HYPRE_BoomerAMGSetLogging(sol, 1);
        } else if (!conv.count("noprint")) {
        ...
        auto max_levels = list.get_int("mg_max_levels");
        HYPRE_BoomerAMGSetMaxLevels(sol, max_levels);
        HYPRE_BoomerAMGSetMaxCoarseSize(sol, list.get_int("mg_coarse_matrix_size"));

        HYPRE_BoomerAMGSetStrongThreshold(sol, list.get_float("mg_strength_threshold"));
        HYPRE_BoomerAMGSetTruncFactor(sol, list.get_float("mg_trunc_factor"));

        HYPRE_BoomerAMGSetNumPaths(sol, list.get_int("mg_num_paths"));

        HYPRE_BoomerAMGSetCoarsenType(sol, list.get_int("mg_coarsening_type"));
        HYPRE_BoomerAMGSetInterpType(sol, list.get_int("mg_interpolation_type"));

        HYPRE_BoomerAMGSetMaxRowSum(sol, list.get_float("mg_max_row_sum"));
        HYPRE_BoomerAMGSetPMaxElmts(sol, list.get_int("mg_Pmax_elements"));
        HYPRE_BoomerAMGSetNonGalerkinTol(sol, list.get_float("mg_nonGalerkin_tol"));

        HYPRE_BoomerAMGSetAggNumLevels(sol, list.get_int("mg_agg_num_levels"));
        HYPRE_BoomerAMGSetAggInterpType(sol, list.get_int("mg_agg_interpolation_type"));

        HYPRE_BoomerAMGSetAggTruncFactor(sol, list.get_float("mg_agg_trunc_factor"));
        HYPRE_BoomerAMGSetAggP12TruncFactor(sol, list.get_float("mg_agg_P12_trunc_factor"));

        HYPRE_BoomerAMGSetAggPMaxElmts(sol, list.get_int("mg_agg_Pmax_elements"));
        HYPRE_BoomerAMGSetAggP12MaxElmts(sol, list.get_int("mg_agg_P12max_elements"));
        ...
        for (int level = 0; level < (int)max_levels; level++) {
            auto per_level_list = global_list.get(solver_type, level);
            auto mg_nonGalerkin_tol = per_level_list.get_float("mg_nonGalerkin_tol");
        ...
        for (const auto &i : solver_types) {
            ...
            std::string method = solver_list.get_string("method");
            if (method == "Jacobi") {
                HYPRE_BoomerAMGSetCycleRelaxType(sol, 0, solver_type_idx);
                HYPRE_BoomerAMGSetRelaxWt(sol, solver_list.get_float("relax_factor"));
                HYPRE_BoomerAMGSetCycleNumSweeps(sol, solver_list.get_int("max_iters"),
                                                 solver_type_idx);
            } else if (method == "HSGS") {
                ...
                HYPRE_BoomerAMGSetChebyOrder(sol, solver_list.get_int("polynomial_order"));
