...
void get_full_hierarchy(matrix::matrix &m, std::vector<matrix::mg_layer> &mg_tree,
    ...
    bool mg_reduced_precision = list.get_bool("mg_reduced_precision");
...
void get_per_level_hierarchy(matrix::matrix &m, std::vector<matrix::mg_layer> &mg_tree,
    ...
    uint32_t mg_max_levels = list.get_int("mg_max_levels");
    uint32_t mg_coarse_matrix_size = list.get_int("mg_coarse_matrix_size");
    uint32_t mg_agg_num_levels = list.get_int("mg_agg_num_levels");
    bool mg_reduced_precision = list.get_bool("mg_reduced_precision");
    ...
    while (!completed) {
        ...
        modified_list.forced_change_value<uint32_t>(solver_type, "mg_max_levels", 2);
        modified_list.forced_change_value<uint32_t>(solver_type, "mg_agg_num_levels", agg_lev);
