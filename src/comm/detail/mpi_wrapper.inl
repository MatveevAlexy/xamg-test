...
template <typename T>
void allreduce_bxor(T *in, T *out, size_t size, scope comm) {
    MPI_ID &mpi_id = *((MPI_ID *)id.mpi_details);
    MPI_Allreduce(in, out, size, mpi_type.type<T>(), MPI_BXOR, mpi_id.get_comm(comm));
}
