...
inline void blas::copy(const vector::vector &x, vector::vector &y) {
    ...
    if (x.sharing_mode == y.sharing_mode) {
        ...
        if ((x.sharing_mode == mem::CORE) && (y.sharing_mode == mem::NUMA_NODE)) {
            ...
            block_size = x.size;
            block_offset = x.ext_offset - y.global_numa_offset(); // y.global_numa_offset() == 18
...
template <typename T, const uint16_t NV>
inline void blas::bxor_global(const vector::vector &x, vector::vector &res) {
    assert(x.nv == NV);
    assert(res.nv == NV);
    x.check(vector::vector::allocated);
    res.check(vector::vector::allocated);
    res.check(vector::vector::nonempty);
    assert(res.get_elem_size() == 8);
    if (x.if_zero) {
    ...
    x.check(vector::vector::initialized);
    const T *XAMG_RESTRICT x_ptr = x.get_aligned_ptr<T>();
    uint64_t *XAMG_RESTRICT res_ptr = res.get_aligned_ptr<uint64_t>();
    ...
    for (uint16_t nv = 0; nv < NV; ++nv)
        res_ptr[nv] = 0.0;
    if (sizeof(T) == 4) {
        uint32_t *XAMG_RESTRICT iptr = (uint32_t * XAMG_RESTRICT) x_ptr;
        XAMG_VECTOR_ALIGN
        for (uint64_t i = NV * core_offset; i < NV * (core_offset + core_size); i += NV) {
            for (uint16_t nv = 0; nv < NV; ++nv)
                res_ptr[nv] ^= (uint64_t)iptr[i + nv];
        }
    } else if (sizeof(T) == 8) {
        uint64_t *XAMG_RESTRICT iptr = (uint64_t * XAMG_RESTRICT) x_ptr;
        XAMG_VECTOR_ALIGN
        for (uint64_t i = NV * core_offset; i < NV * (core_offset + core_size); i += NV) {
            for (uint16_t nv = 0; nv < NV; ++nv)
                res_ptr[nv] ^= iptr[i + nv];
        }
    } else {
        assert(0 && "cannot handle this type for XOR operation.");
    }

    XAMG_STACK_ALIGNMENT_PREFIX uint64_t bxor_global[NV];
    mpi::allreduce_bxor<uint64_t>(res_ptr, bxor_global, NV);
    XAMG_VECTOR_ALIGN
    for (uint16_t m = 0; m < NV; ++m)
        res_ptr[m] = bxor_global[m];
    res.if_zero = false;
    res.if_initialized = true;
}
