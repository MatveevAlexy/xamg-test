...
struct MergedIBiCGStab : public base_solver<F, NV> {
    const uint16_t nvecs = 9;
    const uint16_t comm_size = 7;
    DECLARE_INHERITED_FROM_BASESOLVER(MergedIBiCGStab)
};

template <typename F, uint16_t NV>
void MergedIBiCGStab<F, NV>::matrix_info() {
    if (precond == nullptr) {
        A.info.print("A");
    } else {
        precond->matrix_info();
...
template <typename F, uint16_t NV>
void MergedIBiCGStab<F, NV>::solve(const vector::vector &conv, XAMG::mpi::token &tok) {
    ...
    do {
        ...
        blas::scal<F, NV>(a_1, omega);
        //        io::print_vector_norm<F, NV>(rho1, " rho1 ");
        ...
        //        io::print_vector_norm<F, NV>(delta, " delta ");

        blas::xdivy_z<F, NV>(delta, omega, beta);
        //        io::print_vector_norm<F, NV>(beta, " beta ");
        ...
        blas::specific::merged_ibicgstab_group1<F, NV>(r, z, v, u, q, s, alpha, alpha_1, beta,
                                                       delta);
        ...
        blas::specific::merged_ibicgstab_group2<F, NV>(t, u, q, r0, s, f0, alpha, phi, pi, gamma,
                                                       eta, theta, kappa, nu);
        ...
        blas::specific::merged_ibicgstab_group3<F, NV>(r, s, t, x, z, a1_conv, omega_conv, omega);
        //        io::print_vector_norm<F, NV>(r, "r");
        //        io::print_vector_norm<F, NV>(x, "x");
