...
struct RBiCGStab : public base_solver<F, NV> {
    const uint16_t nvecs = 8;
    const uint16_t comm_size = 4;
    DECLARE_INHERITED_FROM_BASESOLVER(RBiCGStab)
};

template <typename F, uint16_t NV>
void RBiCGStab<F, NV>::matrix_info() {
    if (precond == nullptr) {
        A.info.print("A");
    } else {
        precond->matrix_info();
...
template <typename F, uint16_t NV>
void RBiCGStab<F, NV>::solve(const vector::vector &conv, XAMG::mpi::token &tok) {
    ...
    for (bool last_iter = false; !last_iter;) {
        ...
        //        12. r^ = r - \alpha v
        blas::scal<F, NV>(a_1, alpha);
        blas::axpby<F, NV>(alpha, v, a1, r);
        blas::scal<F, NV>(a_1, alpha);

        //        13. \theta = (t, r^), \phi = (t, t), \psi = (t, r0), \eta = (r^, r^)
        blas::dot<F, NV>(t, r, theta);
        blas::dot<F, NV>(t, t, phi);
        blas::dot<F, NV>(t, r0, psi);
        blas::dot<F, NV>(r, r, eta);
        ...
        ////        23. x = x + \alpha v_  + \omega t_
        //        blas::axpbypcz<F, NV>(alpha, v_, omega, t_, a1, x);

        //        24. z = t_ - \omega z
        blas::scal<F, NV>(a_1, omega);
        blas::axpby<F, NV>(a1, t_, omega, z);
        //        blas::scal<F, NV>(a_1, omega);

        //        25. v_ = z + beta v_ - beta*omega s
        //      order of computations can affect the convergence rate!!!
        //        blas::scal<F, NV>(a_1, omega);
        blas::ax_y<F, NV>(omega, beta, delta);
        blas::scal<F, NV>(a_1, omega);
        blas::axpbypcz<F, NV>(a1, z, delta, s, beta, v_);
