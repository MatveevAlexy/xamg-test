...
struct MergedBiCGStab : public base_solver<F, NV> {
    const uint16_t nvecs = 8;
    const uint16_t comm_size = 3;
    DECLARE_INHERITED_FROM_BASESOLVER(MergedBiCGStab)
};

template <typename F, uint16_t NV>
void MergedBiCGStab<F, NV>::matrix_info() {
    if (precond == nullptr) {
        A.info.print("A");
    } else {
        precond->matrix_info();
...
template <typename F, uint16_t NV>
void MergedBiCGStab<F, NV>::solve(const vector::vector &conv, XAMG::mpi::token &tok) {
    ...
    for (bool last_iter = false; !last_iter;) {
        ...
        blas::ax_y<F, NV>(iconv, alpha, alpha_conv);
        blas::ax_y<F, NV>(iconv, omega, omega_conv);
        blas::specific::merged_bicgstab_group5<F, NV>(x, p, s, r, v, alpha_conv, omega_conv, alpha,
                                                      omega, beta);
