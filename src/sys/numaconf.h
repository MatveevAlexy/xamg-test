...
struct numa_conf {
    ...
    mutable int core = -1;
    int nthreads = 0;
    void init_generic(); // gets ncores and ngpus from system; can't get nnumas,
                         // always sets nnumas=1
    ...
    try {
        if (str[0] == 'n') {
            ...
            for (auto &s : scheme) {
                ...
                if (kv[0] == "ncores") {
                    if (kv[1] == "*") {
                        // FIXME seems to be related to nnumas in fact
                        // issue: #51
...
static inline void numa_conf_check(const numa_conf &conf) {
    ...
    if ((id.gl_nprocs % id.nd_ncores) || (id.gl_nprocs % id.nm_ncores)) {
        XAMG_FATAL_ERROR(
            "Total number of MPI ranks does not match the number of ncores & nnumas specified");
    }
}
