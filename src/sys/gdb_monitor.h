...
class gdb_monitor {
  public:
    ...
    void start(const std::string &_gdb, const std::string &_exe, const std::string &_batch) {
        ...
        if (output == nullptr) {
            std::cerr << "gdb_monitor: ERROR: can't open pipe from child process." << std::endl;
            return;
        }
    ...
    void wait_for_some_output() {
        ...
        if (cnt == 0) {
            std::cerr << "gdb_monitor: ERROR: start: failed at waiting stage, Child process is "
                         "silent."
                      << std::endl;
        }
        usleep(cnt * 10);
        ...
        while (!update(out)) {
            {
                ...
                if (save == "") {
                    ...
                    if (!file_to_save.is_open()) {
                        ...
                        if (!file_to_save.good()) {
                            std::cerr << "gdb_monitor: ERROR: can't open output file for writing."
                                      << std::endl;
                            save = "";
                        }
                    }
    ...
    bool update(std::string &out) {
        std::lock_guard<std::mutex> lk(kill_mtx);
        if (child_pid == 0 || output == nullptr) {
            return true;
        }
        char line[256];
