...
void matrix_block::plot(const std::string &eps_name) const {
    std::ofstream f_out(eps_name);

    int x_0 = data->get_ncols();
    int y_0 = data->get_nrows();
    io::eps::plot_header(x_0, y_0, f_out);
    ...
    for (uint32_t i = 0; i < data->get_nrows(); ++i) {
        data->get_row(i, col, val);
        io::eps::plot_row(i, col, x_0, y_0, f_out);
    }

    io::eps::plot_footer(f_out);
    ...
    for (auto &indx : layer.p2p_comm.sender_indx) {
        if (indx.sharing_mode == mem::CORE) {
            ...
            if (indx.sharing_mode == mem::NUMA) {
                ...
                uint32_t r1 = get_offset_by_indx(indx, offset);
                uint32_t r2 = get_offset_by_indx(indx, offset + size);
