...
struct csr_matrix : backend {
    ...
    virtual uint64_t get_range_size(const uint64_t l1, const uint64_t l2) const {
        // l1 and l2 are the numbers of the rows after the matrix compression
        assert(!if_empty);
    ...
    virtual uint64_t get_uncompressed_range_size(const uint64_t l1, const uint64_t l2) const {
        // l1 and l2 are the numbers of the rows before the matrix compression (for indexed matrices)
        assert(!if_empty);

        row.check(vector::vector::initialized);
        row_ind.check(vector::vector::initialized);
        assert(l1 <= block_nrows);
        assert(l2 <= block_nrows);
        assert(l1 <= l2);

        uint64_t r1 = vector::get_offset_by_indx<I3>(row_ind, l1);
        uint64_t r2 = vector::get_offset_by_indx<I3>(row_ind, l2);

        return (get_range_size(r1, r2));
    };

    virtual uint64_t get_row_size(const uint64_t l) const {
        assert(!if_empty);

        return (get_range_size(l, l + 1));
    }
