...
struct matrix_block {
    ...
    void plot(const std::string &eps_name) const;
};
...
struct matrix {
    ...
    void plot(const std::string eps_name_prefix, size_t mask = numa_diag) {
        if (id.numa_slave_process())
            return;

        // output can be added for the core layer too...
        assert((!(mask & core_diag)) && (!(mask & core_offd)));

        if (data_layer.find(segment::NUMA) != data_layer.end()) {
            const auto &layer = data_layer.find(segment::NUMA)->second;
            if (mask & numa_diag)
                layer.diag.plot(eps_name_prefix + "_numa_diag_proc_" + std::to_string(id.gl_proc) +
                                ".eps");

            if (mask & numa_offd) {
                for (size_t i = 0; i < layer.offd.size(); ++i)
                    layer.offd[i].plot(eps_name_prefix + "_numa_offd_" + std::to_string(i) +
                                       "_proc_" + std::to_string(id.gl_proc) + ".eps");
            }
        }

        if (data_layer.find(segment::NODE) != data_layer.end()) {
            const auto &layer = data_layer.find(segment::NODE)->second;
            if (mask & node_offd) {
                for (size_t i = 0; i < layer.offd.size(); ++i)
                    layer.offd[i].plot(eps_name_prefix + "_node_offd_" + std::to_string(i) +
                                       "_proc_" + std::to_string(id.gl_proc) + ".eps");
            }
        }
    }
