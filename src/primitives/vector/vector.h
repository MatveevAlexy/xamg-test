...
template <typename T>
uint64_t get_offset_by_indx(const vector &ind, const uint64_t l) {
    // this function returns the index of the element, which is greater or equal to l
    ind.check(vector::vector::initialized);

    uint64_t r = 0;
    auto ind_ptr = ind.get_aligned_ptr<T>();
    for (size_t i = 0; i < ind.size; ++i) {
        if ((uint64_t)ind_ptr[r] < l)
            ++r;
    }
...
static inline uint64_t get_offset_by_indx(const indx_vector &ind, const uint64_t l) {
    return get_offset_by_indx<uint32_t>(ind, l);
}
