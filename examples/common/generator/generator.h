...
struct generator_params_t {
    ...
    uint16_t config = 0;               // valid for CUBE case
...
std::string generate_system(XAMG::matrix::csr_matrix<F, I1, I2, I3, I4> &mat,
                            XAMG::vector::vector &x, XAMG::vector::vector &b,
                            const generator_params_t &generator_params) {
    ...
    if (generator_params.test_case == "cube") {
        generate_cube<F, I1, I2, I3, I4, NV>(mat, x, b, generator_params.nx, generator_params.ny,
                                             generator_params.nz, generator_params.config);
        mtx = std::string("@") + std::to_string(generator_params.nx) + "x" +
              std::to_string(generator_params.ny) + "x" + std::to_string(generator_params.nz);
        if (generator_params.config) {
            mtx += "_cfg" + std::to_string(generator_params.config);
        }
        ...
        mtx = std::string("@channel:") + std::to_string(generator_params.scale);
    }
    return mtx;
}
...
template <typename F, typename I1, typename I2, typename I3, typename I4, uint16_t NV>
std::string generate_system(XAMG::matrix::csr_matrix<F, I1, I2, I3, I4> &mat,
                            XAMG::vector::vector &x, XAMG::vector::vector &b, const size_t nx,
