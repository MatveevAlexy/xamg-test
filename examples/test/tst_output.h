...
struct tst_store_output {
    ...
    void store_xamg_vector_norm(const std::string &block, const std::string &key,
        ...
        auto res_ = res.get_element<T>(0);
        std::transform(res_.begin(), res_.end(), res_.begin(), [](float64_t x) { return sqrt(x); });
        YAML::Node &n = nodes[block];
        n[key] = res_;
        n[key].SetStyle(YAML::EmitterStyle::Flow);
    }
    template <typename T, int NV>
    void store_xamg_vector_csum(const std::string &block, const std::string &key,
                                const XAMG::vector::vector &vec) {
        XAMG::vector::vector csum;
        csum.alloc<uint64_t>(1, NV);
        XAMG::blas::bxor_global<T, NV>(vec, csum);
        auto csum_ = csum.get_element<uint64_t>(0);
        YAML::Node &n = nodes[block];
        n[key] = csum_;
        n[key].SetStyle(YAML::EmitterStyle::Flow);
    }
